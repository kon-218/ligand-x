...
from services.qc.tasks import celery_app, run_orca_job_opi, load_results_from_db, calculate_fukui_indices, perform_conformer_search, calculate_bde
...
class QuantumChemistryService:
    ...
    def submit_bde_job(self, job_data: Dict[str, Any]) -> Dict[str, Any]:
        """Submit a Bond Dissociation Energy (BDE) calculation job."""
        if not job_data:
            return {"error": "Missing request body"}, 400
        ...
        try:
            ...
            if not validation_result['valid']:
                error_msg = '; '.join(validation_result['errors'])
                logger.error(f"BDE QC validation failed: {error_msg}")
                return {"error": error_msg}, 400
        except StructureValidationError as e:
            logger.error(f"BDE QC structure validation error: {e}")
            return {"error": str(e)}, 400
        except Exception as e:
            logger.error(f"BDE structure validation exception: {e}", exc_info=True)
            return {"error": f"Structure validation failed: {str(e)}"}, 400
        
        # Validate mode
        mode = job_data.get('mode', 'rapid').lower()
        valid_modes = list(QCConfig.BDE_MODE_PRESETS.keys())
        if mode not in valid_modes:
            return {"error": f"Invalid mode: {mode}. Valid modes: {valid_modes}"}, 400
        
        try:
            logger.info(f"Submitting BDE job with mode={mode}, parameters: {job_data.keys()}")
            task = calculate_bde.delay(job_data)
            
            response = {
                "job_id": task.id,
                "status": "PENDING",
                "message": f"BDE calculation submitted successfully (mode: {mode})",
                "links": {
                    "status": f"/api/qc/jobs/status/{task.id}",
                    "results": f"/api/qc/jobs/results/{task.id}"
                }
            ...
            logger.error(f"Error submitting BDE job: {e}", exc_info=True)
            return {"error": f"Failed to submit job: {str(e)}"}, 500
    ...
    def get_job_status(self, job_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Try to load detailed progress from DB if available
            try:
                db_result = load_results_from_db(job_id)
                if db_result and "progress" in db_result:
                    response["progress"] = db_result["progress"]
            except Exception:
                pass
    ...
    def add_hydrogens(self, molecule_data: str) -> Dict[str, Any]:
        ...
        try:
            ...
            new_xyz = '\n'.join(xyz_lines)
            result = {"molecule_xyz": new_xyz}

            # When input had bond order information (SDF/PDB), also return SDF so the
            # viewer can render explicit bond types (double, triple) alongside the H atoms.
            if input_format in ('SDF', 'PDB'):
                try:
                    molecule_sdf = Chem.MolToMolBlock(mol_with_h)
                    result["molecule_sdf"] = molecule_sdf
                except Exception as e:
                    logger.debug(f"Could not generate SDF output: {e}")
